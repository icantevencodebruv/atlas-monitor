    ...
    def _migrate(self) -> None:
        with self._lock, self._conn:
            ...
            if "original_text" not in cols:
                self._conn.execute("ALTER TABLE transcripts ADD COLUMN original_text TEXT")
    ...
    def add_transcripts(self, segment_id: int, rows: Iterable[dict]) -> None:
        with self._lock, self._conn:
            self._conn.executemany(
                """
                INSERT INTO transcripts (segment_id, start_ts, end_ts, speaker, text, original_text, low_confidence)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                [
                    (
                        ...
                        row["speaker"],
                        row["text"],
                        row.get("original_text", row["text"]),
