...
class EnrollmentService:
    ...
    def _record_with_stop(self, duration_sec: int) -> tuple[np.ndarray, bool]:
        ...
        with self._device_lock:
            preferred_hostapi = self._audio_config.device_hostapi_preference.get(
                "windows" if os.name == "nt" else "macos"
            )
            last_exc = None
            for attempt in range(2):
                select_input_device(preferred_hostapi, self._audio_config.input_device_name)
                try:
                    with sd.InputStream(
                    ...
                    last_exc = exc
                    logger.warning("Enrollment capture failed on attempt %s: %s", attempt + 1, exc)
            if last_exc is not None and captured == 0 and not chunks:
                raise last_exc
